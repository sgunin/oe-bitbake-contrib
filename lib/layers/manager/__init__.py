...
ignore - a text string with a space deliminated list of layerItem names to ignore when downloading."""
        ...
        if not plugin:
            raise NotImplementedError("layer manager fetch %s is not available" % index_fetcher)
    ...
    def unpack(self):
        ...
        plugin = self.get_plugin(self.index_fetcher)
        if not plugin:
            raise NotImplementedError("layer manager unpack %s is not available" % index_fetcher)
